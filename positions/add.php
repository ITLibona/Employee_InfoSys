...
$errors = [];
$data   = ['title' => '', 'department_id' => '', 'salary_grade' => '', 'description' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCSRF($_POST['csrf_token'] ?? '')) {
        ...
        if ($data['salary_grade'] !== '' && (!ctype_digit($data['salary_grade']) || (int)$data['salary_grade'] < 1 || (int)$data['salary_grade'] > 33)) {
            $errors[] = 'Salary grade must be a number between 1 and 33.';
        }

        if (empty($errors)) {
            $stmt = $db->prepare(
                "INSERT INTO positions (title, department_id, salary_grade, description)
                 VALUES (?, ?, ?, ?)"
            );
            $stmt->execute([
                $data['title'],
            ...
            setFlash('success', 'Position "' . $data['title'] . '" created successfully.');
...
<div class="page-header">
    <div>
        <h2><i class="fas fa-plus me-2"></i>Add Position</h2>
                ...
                <form method="POST" action="" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="salary_grade" class="form-control"
                               min="1" max="33" placeholder="1–33"
                               value="<?= e($data['salary_grade']) ?>">
                        <div class="form-text">Philippine government salary grade (1–33).</div>
                    </div>
                    <div class="mb-4">
